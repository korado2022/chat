...
class ServerStorage:
    ...
    def add_contact(self, user, contact):
        ...
        user_id = self.session.query(self.Users).filter_by(name=user).first().id
        contact_id = self.session.query(self.Users).filter_by(name=contact).first().id

        # Проверяем что не дубль и что контакт может существовать (полю пользователь мы доверяем)
        if not contact or self.session.query(self.UsersContacts).filter_by(user_id=user_id,
    ...
    def remove_contact(self, user, contact):
        ...
        self.session.query(self.UsersContacts).filter(
            self.UsersContacts.user_id == user_id,
            self.UsersContacts.contact_id == contact_id
        ).delete()
        self.session.commit()
